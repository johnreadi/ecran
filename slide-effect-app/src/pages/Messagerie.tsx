...
import { Send, Paperclip, Search, Archive, Reply, Trash2, Calendar, X, Download, FileText, Image, File, Menu } from 'lucide-react';
...
export default function MessagerieUser() {
  ...
  const loadConversations = async () => {
    try {
      const response = await fetch(`${SERVER_URL}/messages/conversations?userId=${currentUser.id}`, {
  ...
  const loadMessages = async (userId: string) => {
    try {
      const response = await fetch(`${SERVER_URL}/messages/${userId}?currentUserId=${currentUser.id}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (response.ok) {
        const data = await response.json();
  ...
  const archiveMessage = async (messageId: string, archive: boolean) => {
    try {
      await fetch(`${SERVER_URL}/messages/${messageId}/archive`, {
        method: 'PUT',
        headers: { 
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}` 
        },
        body: JSON.stringify({ userId: currentUser.id, archive })
      });
      loadMessages(selectedUser!.user_id);
      loadConversations();
    } catch (error) {
  ...
  const deleteMessage = async (messageId: string) => {
    ...
    try {
      await fetch(`${SERVER_URL}/messages/${messageId}`, {
        method: 'DELETE',
        headers: { 
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}` 
  ...
  return (
    <div className="flex h-screen bg-white overflow-hidden">
      ...
      <div className={`${sidebarOpen ? 'w-80' : 'w-0'} border-r border-gray-200 flex flex-col transition-all duration-300`}>
                      ...
                      {msg.attachments?.length > 0 && (
                        <div className="mt-2 space-y-1">
                          {msg.attachments.map((att: any, idx: number) => (
                            <a
                              key={idx}
                              href={`${SERVER_URL.replace('/api', '')}${att.path}`}
                              target="_blank"
